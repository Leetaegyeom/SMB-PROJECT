...
class CAM_DRIVING:
    def __init__(self):
        ...
        self.horiz_line_threshold = 10

    def find_midpoint(self):
        while not self.img_proc.is_image_ready():
            RATE.sleep()
        
        left_x, right_x, horiz_lin_num = self.img_proc.find_line()

        if horiz_lin_num > self.horiz_line_threshold:
            return None, 0
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        cam_midpoint, speed = cam_drive.find_midpoint()
        if cam_midpoint is None:
            midpoint, speed = lidar_drive.find_midpoint(), speed
        else:
